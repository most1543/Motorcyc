<?php
    ...
    {  
        if(isset($_SESSION["tradnum"])) {
            $maxx = "SELECT * FROM Buy";
            $max = mysqli_query ($conn, $maxx);
            $receipt = mysqli_num_rows($max);

            $code = $_POST["code"];
            $date = $_POST["date"];
            $insert = "INSERT INTO `buy` (`TradingNum`, `PCode`, `ReceiptNum`, `Date`, `Cash`, `Online`) VALUES ('$tdfromlogin', '$code', '$receipt', '$date', 'T', 'F')";
...
<body translate="no">
    ...
    <div class="container">
        ...
        <?php
            $query = "SELECT Product.* ,Picture.* FROM Picture LEFT JOIN Product ON Picture.PCode = Product.Code";
