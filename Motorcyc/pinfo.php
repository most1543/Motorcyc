<?php
    ...
    { 
        if(isset($_SESSION["tradnum"])) {
            $code = $_GET["id"];
            $date = $_GET["Cdate"];
            $insert = "INSERT INTO `buy` (`TradingNum`, `PCode`, `ReceiptNum`, `Date`, `Cash`, `Online`) VALUES ('$tdfromlogin', '$code', '11111', '$date', 'T', 'F')";
...
<body translate="no">
    ...
    <div class="container">
        ...
        if(!isset($_GET['varname'])) {
            ...
            $query = "SELECT Product.* ,Picture.* FROM Picture LEFT JOIN Product ON Picture.PCode = Product.Code WHERE Code LIKE '$var_value'";
        ...
        <section class="xbox" >
            ...
            <form class='content' method="post" action="pinfo.php?action=add&id=<?php echo $row["Code"]; ?>&Cdate=<?php echo date("Y-m-d");?>">
                <h2><?php echo $row['Pname_Eng'];?></h2>
                <p><?php echo $row['Pname_Thai'];?></p>
                <p>Code: <?php echo $row['Code'];?></p>
                <p>Brand: <?php echo $row['Brand'];?></p>
                <p>Size: <?php echo $row['Size'];?></p>
                <p>Price: <?php echo $row['Price'];?> THB</p>
